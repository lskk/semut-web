		...
		$.get('<?=base_url('maps/get_places?type=11')?>', function(parkings) {
			...
			if($('#parkingplace').is(":checked")){
			  for(i=0;i<parkings.length;i++){
				  ...
				  }else if(parkings[i].ID == '273'){
					parkingmarker.bindPopup(
					  '<img width="320" height="240" src="<?=base_url(); ?>asset/lskkparking/sabuga.jpeg">'
					  +'<br>'
					  +'<b>'+parkings[i].name+'</b><br>'+parkings[i].address+'<br><br>'+parkings[i].description,{maxWidth: 340}
						);
				  }else{
						parkingmarker.bindPopup(
		...
		if($('#parkingplace').is(":checked")){
		  for(i=0;i<data.length;i++){
			...
			if(data[i].ID == '272')
			{
			  ...
			  +'<? } else { ?>'
			  +'</a><br>'
			  +'<font size="5"><a href="https://chart.googleapis.com/chart?chs=300x300&cht=qr&chl='+<?php echo $this->session->userdata('user_id') ?>+'&choe=UTF-8" target="_blank">print</a></font><br>'
			  +'<b>'+data[i].name+'</b><br>'+data[i].address+'<br><br>'+data[i].description,{maxWidth: 340}
			  +'<? } ?>'
				);
